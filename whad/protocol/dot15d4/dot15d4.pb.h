/* Automatically generated nanopb header */
/* Generated by nanopb-1.0.0-dev */

#ifndef PB_DOT15D4_WHAD_PROTOCOL_DOT15D4_DOT15D4_PB_H_INCLUDED
#define PB_DOT15D4_WHAD_PROTOCOL_DOT15D4_DOT15D4_PB_H_INCLUDED
#include <pb.h>

#if PB_PROTO_HEADER_VERSION != 40
#error Regenerate this file with the current version of nanopb generator.
#endif

/* Enum definitions */
typedef enum _dot15d4_Dot15d4Command { /* *
 Low-level commands */
    /* Set Node address. */
    dot15d4_Dot15d4Command_SetNodeAddress = 0,
    /* Sniff packets. */
    dot15d4_Dot15d4Command_Sniff = 1,
    /* Jam packets. */
    dot15d4_Dot15d4Command_Jam = 2,
    /* Energy detection */
    dot15d4_Dot15d4Command_EnergyDetection = 3,
    /* Send packets. */
    dot15d4_Dot15d4Command_Send = 4,
    dot15d4_Dot15d4Command_SendRaw = 5,
    dot15d4_Dot15d4Command_SendInSlot = 18,
    /* End Device mode. */
    dot15d4_Dot15d4Command_EndDeviceMode = 6,
    /* Coordinator mode. */
    dot15d4_Dot15d4Command_CoordinatorMode = 7,
    /* Router mode. */
    dot15d4_Dot15d4Command_RouterMode = 8,
    /* Start and Stop commands shared with node-related mode. */
    dot15d4_Dot15d4Command_Start = 9,
    dot15d4_Dot15d4Command_Stop = 10,
    /* Man-in-the-Middle mode */
    dot15d4_Dot15d4Command_ManInTheMiddle = 11,
    /* Enable frequency hopping command. */
    dot15d4_Dot15d4Command_EnableHop = 12,
    /* Add links commands. */
    dot15d4_Dot15d4Command_AddLinks = 13,
    /* Delete link command. */
    dot15d4_Dot15d4Command_DeleteLink = 14,
    /* set ChannelMap command. */
    dot15d4_Dot15d4Command_ChannelMap = 15,
    /* Write/Modify superframe command. */
    dot15d4_Dot15d4Command_WriteModifySuperframe = 16,
    /* Delete superframe command. */
    dot15d4_Dot15d4Command_DeleteSuperframe = 17
} dot15d4_Dot15d4Command;

typedef enum _dot15d4_Dot15d4MitmRole {
    dot15d4_Dot15d4MitmRole_REACTIVE_JAMMER = 0,
    dot15d4_Dot15d4MitmRole_CORRECTOR = 1
} dot15d4_Dot15d4MitmRole;

typedef enum _dot15d4_AddressType {
    dot15d4_AddressType_SHORT = 0,
    dot15d4_AddressType_EXTENDED = 1
} dot15d4_AddressType;

/* Struct definitions */
typedef struct _dot15d4_SetNodeAddressCmd {
    uint64_t address;
    dot15d4_AddressType address_type;
} dot15d4_SetNodeAddressCmd;

typedef struct _dot15d4_SniffCmd {
    /* Channel must be specified, the device will only
listen on this specific channel. */
    uint32_t channel;
} dot15d4_SniffCmd;

/* *
 EnergyDetectionCmd

 Enable energy detection mode. */
typedef struct _dot15d4_EnergyDetectionCmd {
    uint32_t channel;
} dot15d4_EnergyDetectionCmd;

typedef struct _dot15d4_JamCmd {
    uint32_t channel;
} dot15d4_JamCmd;

typedef PB_BYTES_ARRAY_T(255) dot15d4_SendCmd_pdu_t;
/* *
 SendCmd

 Transmit 802.15.4 packets on a single channel. */
typedef struct _dot15d4_SendCmd {
    uint32_t channel;
    dot15d4_SendCmd_pdu_t pdu;
} dot15d4_SendCmd;

typedef PB_BYTES_ARRAY_T(255) dot15d4_SendRawCmd_pdu_t;
typedef struct _dot15d4_SendRawCmd {
    uint32_t channel;
    dot15d4_SendRawCmd_pdu_t pdu;
    uint32_t fcs;
} dot15d4_SendRawCmd;

typedef PB_BYTES_ARRAY_T(255) dot15d4_SendInSlotCmd_pdu_t;
/* *
 SendInSlotCmd
 Sends 802.15.4 packets on a single channel on a specified slot corresponding to the wirelessHART protocol.. */
typedef struct _dot15d4_SendInSlotCmd {
    uint64_t slot;
    uint64_t wait_offset;
    dot15d4_SendInSlotCmd_pdu_t pdu;
} dot15d4_SendInSlotCmd;

/* *
 EndDeviceCmd

 Enable End Device mode. */
typedef struct _dot15d4_EndDeviceCmd {
    uint32_t channel;
} dot15d4_EndDeviceCmd;

/* *
 RouterCmd

 Enable Router mode. */
typedef struct _dot15d4_RouterCmd {
    uint32_t channel;
} dot15d4_RouterCmd;

/* *
 CoordinatorCmd

 Enable Coordinator mode. */
typedef struct _dot15d4_CoordinatorCmd {
    uint32_t channel;
} dot15d4_CoordinatorCmd;

/* *
 StartCmd

 Enable node-related modes. */
typedef struct _dot15d4_StartCmd {
    char dummy_field;
} dot15d4_StartCmd;

/* *
 StopCmd

 Disable node-related modes. */
typedef struct _dot15d4_StopCmd {
    char dummy_field;
} dot15d4_StopCmd;

/* *
 SyncCmd */
typedef struct _dot15d4_SyncCmd {
    uint64_t timestamp;
    uint64_t asn;
} dot15d4_SyncCmd;

typedef struct _dot15d4_ManInTheMiddleCmd {
    dot15d4_Dot15d4MitmRole role;
} dot15d4_ManInTheMiddleCmd;

typedef struct _dot15d4_Jammed {
    uint64_t timestamp;
} dot15d4_Jammed;

/* *
 EnableHopCmd

 Enable frequency hopping */
typedef struct _dot15d4_EnableHopCmd {
    bool hopping;
} dot15d4_EnableHopCmd;

typedef PB_BYTES_ARRAY_T(255) dot15d4_AddLinksCmd_links_t;
/* *
 AddLinksCmd
 This command adds the links */
typedef struct _dot15d4_AddLinksCmd {
    uint32_t nb_links;
    dot15d4_AddLinksCmd_links_t links;
} dot15d4_AddLinksCmd;

/* * 
 Delete the corresponding link */
typedef struct _dot15d4_DeleteLinkCmd {
    uint32_t superframeId;
    uint32_t slotNumber;
    uint32_t neighbor;
} dot15d4_DeleteLinkCmd;

/* *
 Add/Modify the channel map */
typedef struct _dot15d4_ChannelMapCmd {
    uint32_t channelMap;
} dot15d4_ChannelMapCmd;

/* *
 Add new superframe or modify existing one */
typedef struct _dot15d4_WriteModifySuperframeCmd {
    uint32_t superframeId;
    uint32_t numberOfSlots;
    uint32_t flags;
    bool has_asn;
    uint64_t asn;
} dot15d4_WriteModifySuperframeCmd;

/* *
 Delete existing superframe */
typedef struct _dot15d4_DeleteSuperframeCmd {
    uint32_t superframeId;
} dot15d4_DeleteSuperframeCmd;

typedef struct _dot15d4_EnergyDetectionSample {
    uint32_t sample;
    uint64_t timestamp;
} dot15d4_EnergyDetectionSample;

typedef PB_BYTES_ARRAY_T(255) dot15d4_RawPduReceived_pdu_t;
typedef struct _dot15d4_RawPduReceived {
    uint32_t channel;
    bool has_rssi;
    int32_t rssi;
    bool has_timestamp;
    uint64_t timestamp;
    bool has_fcs_validity;
    bool fcs_validity;
    dot15d4_RawPduReceived_pdu_t pdu;
    uint32_t fcs;
    bool has_lqi;
    uint32_t lqi;
} dot15d4_RawPduReceived;

typedef PB_BYTES_ARRAY_T(255) dot15d4_PduReceived_pdu_t;
typedef struct _dot15d4_PduReceived {
    uint32_t channel;
    bool has_rssi;
    int32_t rssi;
    bool has_timestamp;
    uint64_t timestamp;
    bool has_fcs_validity;
    bool fcs_validity;
    dot15d4_PduReceived_pdu_t pdu;
    bool has_lqi;
    uint32_t lqi;
} dot15d4_PduReceived;

typedef PB_BYTES_ARRAY_T(255) dot15d4_DiscoveredCommunication_pdu_t;
/* *
 DiscoveredCommunication

 Notifies the discovery of a communication on an unknown link
 This message is to be used to discover existing links and add them to the superframes */
typedef struct _dot15d4_DiscoveredCommunication {
    uint32_t slot;
    uint32_t offset;
    dot15d4_DiscoveredCommunication_pdu_t pdu;
} dot15d4_DiscoveredCommunication;

typedef struct _dot15d4_Message {
    pb_size_t which_msg;
    union _dot15d4_Message_msg {
        /* Messages */
        dot15d4_SetNodeAddressCmd set_node_addr;
        dot15d4_SniffCmd sniff;
        dot15d4_JamCmd jam;
        dot15d4_EnergyDetectionCmd ed;
        dot15d4_SendCmd send;
        dot15d4_SendRawCmd send_raw;
        dot15d4_EndDeviceCmd end_device;
        dot15d4_RouterCmd router;
        dot15d4_CoordinatorCmd coordinator;
        dot15d4_StartCmd start;
        dot15d4_StopCmd stop;
        dot15d4_ManInTheMiddleCmd mitm;
        /* Notifications */
        dot15d4_Jammed jammed;
        dot15d4_EnergyDetectionSample ed_sample;
        dot15d4_RawPduReceived raw_pdu;
        dot15d4_PduReceived pdu;
        dot15d4_EnableHopCmd hopping;
        dot15d4_AddLinksCmd addLinks;
        dot15d4_DeleteLinkCmd deleteLink;
        dot15d4_ChannelMapCmd channelMap;
        dot15d4_WriteModifySuperframeCmd writeModifySuperframeCmd;
        dot15d4_DeleteSuperframeCmd deleteSuperframeCmd;
        dot15d4_DiscoveredCommunication discovered_communication;
        dot15d4_SendInSlotCmd send_in_slot;
        dot15d4_SyncCmd sync;
    } msg;
} dot15d4_Message;


#ifdef __cplusplus
extern "C" {
#endif

/* Helper constants for enums */
#define _dot15d4_Dot15d4Command_MIN dot15d4_Dot15d4Command_SetNodeAddress
#define _dot15d4_Dot15d4Command_MAX dot15d4_Dot15d4Command_SendInSlot
#define _dot15d4_Dot15d4Command_ARRAYSIZE ((dot15d4_Dot15d4Command)(dot15d4_Dot15d4Command_SendInSlot+1))

#define _dot15d4_Dot15d4MitmRole_MIN dot15d4_Dot15d4MitmRole_REACTIVE_JAMMER
#define _dot15d4_Dot15d4MitmRole_MAX dot15d4_Dot15d4MitmRole_CORRECTOR
#define _dot15d4_Dot15d4MitmRole_ARRAYSIZE ((dot15d4_Dot15d4MitmRole)(dot15d4_Dot15d4MitmRole_CORRECTOR+1))

#define _dot15d4_AddressType_MIN dot15d4_AddressType_SHORT
#define _dot15d4_AddressType_MAX dot15d4_AddressType_EXTENDED
#define _dot15d4_AddressType_ARRAYSIZE ((dot15d4_AddressType)(dot15d4_AddressType_EXTENDED+1))

#define dot15d4_SetNodeAddressCmd_address_type_ENUMTYPE dot15d4_AddressType













#define dot15d4_ManInTheMiddleCmd_role_ENUMTYPE dot15d4_Dot15d4MitmRole














/* Initializer values for message structs */
#define dot15d4_SetNodeAddressCmd_init_default   {0, _dot15d4_AddressType_MIN}
#define dot15d4_SniffCmd_init_default            {0}
#define dot15d4_EnergyDetectionCmd_init_default  {0}
#define dot15d4_JamCmd_init_default              {0}
#define dot15d4_SendCmd_init_default             {0, {0, {0}}}
#define dot15d4_SendRawCmd_init_default          {0, {0, {0}}, 0}
#define dot15d4_SendInSlotCmd_init_default       {0, 0, {0, {0}}}
#define dot15d4_EndDeviceCmd_init_default        {0}
#define dot15d4_RouterCmd_init_default           {0}
#define dot15d4_CoordinatorCmd_init_default      {0}
#define dot15d4_StartCmd_init_default            {0}
#define dot15d4_StopCmd_init_default             {0}
#define dot15d4_SyncCmd_init_default             {0, 0}
#define dot15d4_ManInTheMiddleCmd_init_default   {_dot15d4_Dot15d4MitmRole_MIN}
#define dot15d4_Jammed_init_default              {0}
#define dot15d4_EnableHopCmd_init_default        {0}
#define dot15d4_AddLinksCmd_init_default         {0, {0, {0}}}
#define dot15d4_DeleteLinkCmd_init_default       {0, 0, 0}
#define dot15d4_ChannelMapCmd_init_default       {0}
#define dot15d4_WriteModifySuperframeCmd_init_default {0, 0, 0, false, 0}
#define dot15d4_DeleteSuperframeCmd_init_default {0}
#define dot15d4_EnergyDetectionSample_init_default {0, 0}
#define dot15d4_RawPduReceived_init_default      {0, false, 0, false, 0, false, 0, {0, {0}}, 0, false, 0}
#define dot15d4_PduReceived_init_default         {0, false, 0, false, 0, false, 0, {0, {0}}, false, 0}
#define dot15d4_DiscoveredCommunication_init_default {0, 0, {0, {0}}}
#define dot15d4_Message_init_default             {0, {dot15d4_SetNodeAddressCmd_init_default}}
#define dot15d4_SetNodeAddressCmd_init_zero      {0, _dot15d4_AddressType_MIN}
#define dot15d4_SniffCmd_init_zero               {0}
#define dot15d4_EnergyDetectionCmd_init_zero     {0}
#define dot15d4_JamCmd_init_zero                 {0}
#define dot15d4_SendCmd_init_zero                {0, {0, {0}}}
#define dot15d4_SendRawCmd_init_zero             {0, {0, {0}}, 0}
#define dot15d4_SendInSlotCmd_init_zero          {0, 0, {0, {0}}}
#define dot15d4_EndDeviceCmd_init_zero           {0}
#define dot15d4_RouterCmd_init_zero              {0}
#define dot15d4_CoordinatorCmd_init_zero         {0}
#define dot15d4_StartCmd_init_zero               {0}
#define dot15d4_StopCmd_init_zero                {0}
#define dot15d4_SyncCmd_init_zero                {0, 0}
#define dot15d4_ManInTheMiddleCmd_init_zero      {_dot15d4_Dot15d4MitmRole_MIN}
#define dot15d4_Jammed_init_zero                 {0}
#define dot15d4_EnableHopCmd_init_zero           {0}
#define dot15d4_AddLinksCmd_init_zero            {0, {0, {0}}}
#define dot15d4_DeleteLinkCmd_init_zero          {0, 0, 0}
#define dot15d4_ChannelMapCmd_init_zero          {0}
#define dot15d4_WriteModifySuperframeCmd_init_zero {0, 0, 0, false, 0}
#define dot15d4_DeleteSuperframeCmd_init_zero    {0}
#define dot15d4_EnergyDetectionSample_init_zero  {0, 0}
#define dot15d4_RawPduReceived_init_zero         {0, false, 0, false, 0, false, 0, {0, {0}}, 0, false, 0}
#define dot15d4_PduReceived_init_zero            {0, false, 0, false, 0, false, 0, {0, {0}}, false, 0}
#define dot15d4_DiscoveredCommunication_init_zero {0, 0, {0, {0}}}
#define dot15d4_Message_init_zero                {0, {dot15d4_SetNodeAddressCmd_init_zero}}

/* Field tags (for use in manual encoding/decoding) */
#define dot15d4_SetNodeAddressCmd_address_tag    1
#define dot15d4_SetNodeAddressCmd_address_type_tag 2
#define dot15d4_SniffCmd_channel_tag             1
#define dot15d4_EnergyDetectionCmd_channel_tag   1
#define dot15d4_JamCmd_channel_tag               1
#define dot15d4_SendCmd_channel_tag              1
#define dot15d4_SendCmd_pdu_tag                  2
#define dot15d4_SendRawCmd_channel_tag           1
#define dot15d4_SendRawCmd_pdu_tag               2
#define dot15d4_SendRawCmd_fcs_tag               3
#define dot15d4_SendInSlotCmd_slot_tag           1
#define dot15d4_SendInSlotCmd_wait_offset_tag    2
#define dot15d4_SendInSlotCmd_pdu_tag            3
#define dot15d4_EndDeviceCmd_channel_tag         1
#define dot15d4_RouterCmd_channel_tag            1
#define dot15d4_CoordinatorCmd_channel_tag       1
#define dot15d4_SyncCmd_timestamp_tag            1
#define dot15d4_SyncCmd_asn_tag                  2
#define dot15d4_ManInTheMiddleCmd_role_tag       1
#define dot15d4_Jammed_timestamp_tag             1
#define dot15d4_EnableHopCmd_hopping_tag         1
#define dot15d4_AddLinksCmd_nb_links_tag         1
#define dot15d4_AddLinksCmd_links_tag            2
#define dot15d4_DeleteLinkCmd_superframeId_tag   1
#define dot15d4_DeleteLinkCmd_slotNumber_tag     2
#define dot15d4_DeleteLinkCmd_neighbor_tag       3
#define dot15d4_ChannelMapCmd_channelMap_tag     1
#define dot15d4_WriteModifySuperframeCmd_superframeId_tag 1
#define dot15d4_WriteModifySuperframeCmd_numberOfSlots_tag 2
#define dot15d4_WriteModifySuperframeCmd_flags_tag 3
#define dot15d4_WriteModifySuperframeCmd_asn_tag 4
#define dot15d4_DeleteSuperframeCmd_superframeId_tag 1
#define dot15d4_EnergyDetectionSample_sample_tag 1
#define dot15d4_EnergyDetectionSample_timestamp_tag 2
#define dot15d4_RawPduReceived_channel_tag       1
#define dot15d4_RawPduReceived_rssi_tag          2
#define dot15d4_RawPduReceived_timestamp_tag     3
#define dot15d4_RawPduReceived_fcs_validity_tag  4
#define dot15d4_RawPduReceived_pdu_tag           5
#define dot15d4_RawPduReceived_fcs_tag           6
#define dot15d4_RawPduReceived_lqi_tag           7
#define dot15d4_PduReceived_channel_tag          1
#define dot15d4_PduReceived_rssi_tag             2
#define dot15d4_PduReceived_timestamp_tag        3
#define dot15d4_PduReceived_fcs_validity_tag     4
#define dot15d4_PduReceived_pdu_tag              5
#define dot15d4_PduReceived_lqi_tag              6
#define dot15d4_DiscoveredCommunication_slot_tag 1
#define dot15d4_DiscoveredCommunication_offset_tag 2
#define dot15d4_DiscoveredCommunication_pdu_tag  3
#define dot15d4_Message_set_node_addr_tag        1
#define dot15d4_Message_sniff_tag                2
#define dot15d4_Message_jam_tag                  3
#define dot15d4_Message_ed_tag                   4
#define dot15d4_Message_send_tag                 5
#define dot15d4_Message_send_raw_tag             6
#define dot15d4_Message_end_device_tag           7
#define dot15d4_Message_router_tag               8
#define dot15d4_Message_coordinator_tag          9
#define dot15d4_Message_start_tag                10
#define dot15d4_Message_stop_tag                 11
#define dot15d4_Message_mitm_tag                 12
#define dot15d4_Message_jammed_tag               13
#define dot15d4_Message_ed_sample_tag            14
#define dot15d4_Message_raw_pdu_tag              15
#define dot15d4_Message_pdu_tag                  16
#define dot15d4_Message_hopping_tag              17
#define dot15d4_Message_addLinks_tag             18
#define dot15d4_Message_deleteLink_tag           19
#define dot15d4_Message_channelMap_tag           20
#define dot15d4_Message_writeModifySuperframeCmd_tag 21
#define dot15d4_Message_deleteSuperframeCmd_tag  22
#define dot15d4_Message_discovered_communication_tag 23
#define dot15d4_Message_send_in_slot_tag         24
#define dot15d4_Message_sync_tag                 25

/* Struct field encoding specification for nanopb */
#define dot15d4_SetNodeAddressCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT64,   address,           1) \
X(a, STATIC,   SINGULAR, UENUM,    address_type,      2)
#define dot15d4_SetNodeAddressCmd_CALLBACK NULL
#define dot15d4_SetNodeAddressCmd_DEFAULT NULL

#define dot15d4_SniffCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1)
#define dot15d4_SniffCmd_CALLBACK NULL
#define dot15d4_SniffCmd_DEFAULT NULL

#define dot15d4_EnergyDetectionCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1)
#define dot15d4_EnergyDetectionCmd_CALLBACK NULL
#define dot15d4_EnergyDetectionCmd_DEFAULT NULL

#define dot15d4_JamCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1)
#define dot15d4_JamCmd_CALLBACK NULL
#define dot15d4_JamCmd_DEFAULT NULL

#define dot15d4_SendCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1) \
X(a, STATIC,   SINGULAR, BYTES,    pdu,               2)
#define dot15d4_SendCmd_CALLBACK NULL
#define dot15d4_SendCmd_DEFAULT NULL

#define dot15d4_SendRawCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1) \
X(a, STATIC,   SINGULAR, BYTES,    pdu,               2) \
X(a, STATIC,   SINGULAR, UINT32,   fcs,               3)
#define dot15d4_SendRawCmd_CALLBACK NULL
#define dot15d4_SendRawCmd_DEFAULT NULL

#define dot15d4_SendInSlotCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT64,   slot,              1) \
X(a, STATIC,   SINGULAR, UINT64,   wait_offset,       2) \
X(a, STATIC,   SINGULAR, BYTES,    pdu,               3)
#define dot15d4_SendInSlotCmd_CALLBACK NULL
#define dot15d4_SendInSlotCmd_DEFAULT NULL

#define dot15d4_EndDeviceCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1)
#define dot15d4_EndDeviceCmd_CALLBACK NULL
#define dot15d4_EndDeviceCmd_DEFAULT NULL

#define dot15d4_RouterCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1)
#define dot15d4_RouterCmd_CALLBACK NULL
#define dot15d4_RouterCmd_DEFAULT NULL

#define dot15d4_CoordinatorCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1)
#define dot15d4_CoordinatorCmd_CALLBACK NULL
#define dot15d4_CoordinatorCmd_DEFAULT NULL

#define dot15d4_StartCmd_FIELDLIST(X, a) \

#define dot15d4_StartCmd_CALLBACK NULL
#define dot15d4_StartCmd_DEFAULT NULL

#define dot15d4_StopCmd_FIELDLIST(X, a) \

#define dot15d4_StopCmd_CALLBACK NULL
#define dot15d4_StopCmd_DEFAULT NULL

#define dot15d4_SyncCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT64,   timestamp,         1) \
X(a, STATIC,   SINGULAR, UINT64,   asn,               2)
#define dot15d4_SyncCmd_CALLBACK NULL
#define dot15d4_SyncCmd_DEFAULT NULL

#define dot15d4_ManInTheMiddleCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UENUM,    role,              1)
#define dot15d4_ManInTheMiddleCmd_CALLBACK NULL
#define dot15d4_ManInTheMiddleCmd_DEFAULT NULL

#define dot15d4_Jammed_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT64,   timestamp,         1)
#define dot15d4_Jammed_CALLBACK NULL
#define dot15d4_Jammed_DEFAULT NULL

#define dot15d4_EnableHopCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, BOOL,     hopping,           1)
#define dot15d4_EnableHopCmd_CALLBACK NULL
#define dot15d4_EnableHopCmd_DEFAULT NULL

#define dot15d4_AddLinksCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   nb_links,          1) \
X(a, STATIC,   SINGULAR, BYTES,    links,             2)
#define dot15d4_AddLinksCmd_CALLBACK NULL
#define dot15d4_AddLinksCmd_DEFAULT NULL

#define dot15d4_DeleteLinkCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   superframeId,      1) \
X(a, STATIC,   SINGULAR, UINT32,   slotNumber,        2) \
X(a, STATIC,   SINGULAR, UINT32,   neighbor,          3)
#define dot15d4_DeleteLinkCmd_CALLBACK NULL
#define dot15d4_DeleteLinkCmd_DEFAULT NULL

#define dot15d4_ChannelMapCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channelMap,        1)
#define dot15d4_ChannelMapCmd_CALLBACK NULL
#define dot15d4_ChannelMapCmd_DEFAULT NULL

#define dot15d4_WriteModifySuperframeCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   superframeId,      1) \
X(a, STATIC,   SINGULAR, UINT32,   numberOfSlots,     2) \
X(a, STATIC,   SINGULAR, UINT32,   flags,             3) \
X(a, STATIC,   OPTIONAL, UINT64,   asn,               4)
#define dot15d4_WriteModifySuperframeCmd_CALLBACK NULL
#define dot15d4_WriteModifySuperframeCmd_DEFAULT NULL

#define dot15d4_DeleteSuperframeCmd_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   superframeId,      1)
#define dot15d4_DeleteSuperframeCmd_CALLBACK NULL
#define dot15d4_DeleteSuperframeCmd_DEFAULT NULL

#define dot15d4_EnergyDetectionSample_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   sample,            1) \
X(a, STATIC,   SINGULAR, UINT64,   timestamp,         2)
#define dot15d4_EnergyDetectionSample_CALLBACK NULL
#define dot15d4_EnergyDetectionSample_DEFAULT NULL

#define dot15d4_RawPduReceived_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1) \
X(a, STATIC,   OPTIONAL, INT32,    rssi,              2) \
X(a, STATIC,   OPTIONAL, UINT64,   timestamp,         3) \
X(a, STATIC,   OPTIONAL, BOOL,     fcs_validity,      4) \
X(a, STATIC,   SINGULAR, BYTES,    pdu,               5) \
X(a, STATIC,   SINGULAR, UINT32,   fcs,               6) \
X(a, STATIC,   OPTIONAL, UINT32,   lqi,               7)
#define dot15d4_RawPduReceived_CALLBACK NULL
#define dot15d4_RawPduReceived_DEFAULT NULL

#define dot15d4_PduReceived_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   channel,           1) \
X(a, STATIC,   OPTIONAL, INT32,    rssi,              2) \
X(a, STATIC,   OPTIONAL, UINT64,   timestamp,         3) \
X(a, STATIC,   OPTIONAL, BOOL,     fcs_validity,      4) \
X(a, STATIC,   SINGULAR, BYTES,    pdu,               5) \
X(a, STATIC,   OPTIONAL, UINT32,   lqi,               6)
#define dot15d4_PduReceived_CALLBACK NULL
#define dot15d4_PduReceived_DEFAULT NULL

#define dot15d4_DiscoveredCommunication_FIELDLIST(X, a) \
X(a, STATIC,   SINGULAR, UINT32,   slot,              1) \
X(a, STATIC,   SINGULAR, UINT32,   offset,            2) \
X(a, STATIC,   SINGULAR, BYTES,    pdu,               3)
#define dot15d4_DiscoveredCommunication_CALLBACK NULL
#define dot15d4_DiscoveredCommunication_DEFAULT NULL

#define dot15d4_Message_FIELDLIST(X, a) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,set_node_addr,msg.set_node_addr),   1) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,sniff,msg.sniff),   2) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,jam,msg.jam),   3) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,ed,msg.ed),   4) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,send,msg.send),   5) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,send_raw,msg.send_raw),   6) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,end_device,msg.end_device),   7) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,router,msg.router),   8) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,coordinator,msg.coordinator),   9) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,start,msg.start),  10) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,stop,msg.stop),  11) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,mitm,msg.mitm),  12) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,jammed,msg.jammed),  13) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,ed_sample,msg.ed_sample),  14) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,raw_pdu,msg.raw_pdu),  15) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,pdu,msg.pdu),  16) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,hopping,msg.hopping),  17) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,addLinks,msg.addLinks),  18) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,deleteLink,msg.deleteLink),  19) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,channelMap,msg.channelMap),  20) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,writeModifySuperframeCmd,msg.writeModifySuperframeCmd),  21) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,deleteSuperframeCmd,msg.deleteSuperframeCmd),  22) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,discovered_communication,msg.discovered_communication),  23) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,send_in_slot,msg.send_in_slot),  24) \
X(a, STATIC,   ONEOF,    MESSAGE,  (msg,sync,msg.sync),  25)
#define dot15d4_Message_CALLBACK NULL
#define dot15d4_Message_DEFAULT NULL
#define dot15d4_Message_msg_set_node_addr_MSGTYPE dot15d4_SetNodeAddressCmd
#define dot15d4_Message_msg_sniff_MSGTYPE dot15d4_SniffCmd
#define dot15d4_Message_msg_jam_MSGTYPE dot15d4_JamCmd
#define dot15d4_Message_msg_ed_MSGTYPE dot15d4_EnergyDetectionCmd
#define dot15d4_Message_msg_send_MSGTYPE dot15d4_SendCmd
#define dot15d4_Message_msg_send_raw_MSGTYPE dot15d4_SendRawCmd
#define dot15d4_Message_msg_end_device_MSGTYPE dot15d4_EndDeviceCmd
#define dot15d4_Message_msg_router_MSGTYPE dot15d4_RouterCmd
#define dot15d4_Message_msg_coordinator_MSGTYPE dot15d4_CoordinatorCmd
#define dot15d4_Message_msg_start_MSGTYPE dot15d4_StartCmd
#define dot15d4_Message_msg_stop_MSGTYPE dot15d4_StopCmd
#define dot15d4_Message_msg_mitm_MSGTYPE dot15d4_ManInTheMiddleCmd
#define dot15d4_Message_msg_jammed_MSGTYPE dot15d4_Jammed
#define dot15d4_Message_msg_ed_sample_MSGTYPE dot15d4_EnergyDetectionSample
#define dot15d4_Message_msg_raw_pdu_MSGTYPE dot15d4_RawPduReceived
#define dot15d4_Message_msg_pdu_MSGTYPE dot15d4_PduReceived
#define dot15d4_Message_msg_hopping_MSGTYPE dot15d4_EnableHopCmd
#define dot15d4_Message_msg_addLinks_MSGTYPE dot15d4_AddLinksCmd
#define dot15d4_Message_msg_deleteLink_MSGTYPE dot15d4_DeleteLinkCmd
#define dot15d4_Message_msg_channelMap_MSGTYPE dot15d4_ChannelMapCmd
#define dot15d4_Message_msg_writeModifySuperframeCmd_MSGTYPE dot15d4_WriteModifySuperframeCmd
#define dot15d4_Message_msg_deleteSuperframeCmd_MSGTYPE dot15d4_DeleteSuperframeCmd
#define dot15d4_Message_msg_discovered_communication_MSGTYPE dot15d4_DiscoveredCommunication
#define dot15d4_Message_msg_send_in_slot_MSGTYPE dot15d4_SendInSlotCmd
#define dot15d4_Message_msg_sync_MSGTYPE dot15d4_SyncCmd

extern const pb_msgdesc_t dot15d4_SetNodeAddressCmd_msg;
extern const pb_msgdesc_t dot15d4_SniffCmd_msg;
extern const pb_msgdesc_t dot15d4_EnergyDetectionCmd_msg;
extern const pb_msgdesc_t dot15d4_JamCmd_msg;
extern const pb_msgdesc_t dot15d4_SendCmd_msg;
extern const pb_msgdesc_t dot15d4_SendRawCmd_msg;
extern const pb_msgdesc_t dot15d4_SendInSlotCmd_msg;
extern const pb_msgdesc_t dot15d4_EndDeviceCmd_msg;
extern const pb_msgdesc_t dot15d4_RouterCmd_msg;
extern const pb_msgdesc_t dot15d4_CoordinatorCmd_msg;
extern const pb_msgdesc_t dot15d4_StartCmd_msg;
extern const pb_msgdesc_t dot15d4_StopCmd_msg;
extern const pb_msgdesc_t dot15d4_SyncCmd_msg;
extern const pb_msgdesc_t dot15d4_ManInTheMiddleCmd_msg;
extern const pb_msgdesc_t dot15d4_Jammed_msg;
extern const pb_msgdesc_t dot15d4_EnableHopCmd_msg;
extern const pb_msgdesc_t dot15d4_AddLinksCmd_msg;
extern const pb_msgdesc_t dot15d4_DeleteLinkCmd_msg;
extern const pb_msgdesc_t dot15d4_ChannelMapCmd_msg;
extern const pb_msgdesc_t dot15d4_WriteModifySuperframeCmd_msg;
extern const pb_msgdesc_t dot15d4_DeleteSuperframeCmd_msg;
extern const pb_msgdesc_t dot15d4_EnergyDetectionSample_msg;
extern const pb_msgdesc_t dot15d4_RawPduReceived_msg;
extern const pb_msgdesc_t dot15d4_PduReceived_msg;
extern const pb_msgdesc_t dot15d4_DiscoveredCommunication_msg;
extern const pb_msgdesc_t dot15d4_Message_msg;

/* Defines for backwards compatibility with code written before nanopb-0.4.0 */
#define dot15d4_SetNodeAddressCmd_fields &dot15d4_SetNodeAddressCmd_msg
#define dot15d4_SniffCmd_fields &dot15d4_SniffCmd_msg
#define dot15d4_EnergyDetectionCmd_fields &dot15d4_EnergyDetectionCmd_msg
#define dot15d4_JamCmd_fields &dot15d4_JamCmd_msg
#define dot15d4_SendCmd_fields &dot15d4_SendCmd_msg
#define dot15d4_SendRawCmd_fields &dot15d4_SendRawCmd_msg
#define dot15d4_SendInSlotCmd_fields &dot15d4_SendInSlotCmd_msg
#define dot15d4_EndDeviceCmd_fields &dot15d4_EndDeviceCmd_msg
#define dot15d4_RouterCmd_fields &dot15d4_RouterCmd_msg
#define dot15d4_CoordinatorCmd_fields &dot15d4_CoordinatorCmd_msg
#define dot15d4_StartCmd_fields &dot15d4_StartCmd_msg
#define dot15d4_StopCmd_fields &dot15d4_StopCmd_msg
#define dot15d4_SyncCmd_fields &dot15d4_SyncCmd_msg
#define dot15d4_ManInTheMiddleCmd_fields &dot15d4_ManInTheMiddleCmd_msg
#define dot15d4_Jammed_fields &dot15d4_Jammed_msg
#define dot15d4_EnableHopCmd_fields &dot15d4_EnableHopCmd_msg
#define dot15d4_AddLinksCmd_fields &dot15d4_AddLinksCmd_msg
#define dot15d4_DeleteLinkCmd_fields &dot15d4_DeleteLinkCmd_msg
#define dot15d4_ChannelMapCmd_fields &dot15d4_ChannelMapCmd_msg
#define dot15d4_WriteModifySuperframeCmd_fields &dot15d4_WriteModifySuperframeCmd_msg
#define dot15d4_DeleteSuperframeCmd_fields &dot15d4_DeleteSuperframeCmd_msg
#define dot15d4_EnergyDetectionSample_fields &dot15d4_EnergyDetectionSample_msg
#define dot15d4_RawPduReceived_fields &dot15d4_RawPduReceived_msg
#define dot15d4_PduReceived_fields &dot15d4_PduReceived_msg
#define dot15d4_DiscoveredCommunication_fields &dot15d4_DiscoveredCommunication_msg
#define dot15d4_Message_fields &dot15d4_Message_msg

/* Maximum encoded size of messages (where known) */
#define DOT15D4_WHAD_PROTOCOL_DOT15D4_DOT15D4_PB_H_MAX_SIZE dot15d4_Message_size
#define dot15d4_AddLinksCmd_size                 264
#define dot15d4_ChannelMapCmd_size               6
#define dot15d4_CoordinatorCmd_size              6
#define dot15d4_DeleteLinkCmd_size               18
#define dot15d4_DeleteSuperframeCmd_size         6
#define dot15d4_DiscoveredCommunication_size     270
#define dot15d4_EnableHopCmd_size                2
#define dot15d4_EndDeviceCmd_size                6
#define dot15d4_EnergyDetectionCmd_size          6
#define dot15d4_EnergyDetectionSample_size       17
#define dot15d4_JamCmd_size                      6
#define dot15d4_Jammed_size                      11
#define dot15d4_ManInTheMiddleCmd_size           2
#define dot15d4_Message_size                     303
#define dot15d4_PduReceived_size                 294
#define dot15d4_RawPduReceived_size              300
#define dot15d4_RouterCmd_size                   6
#define dot15d4_SendCmd_size                     264
#define dot15d4_SendInSlotCmd_size               280
#define dot15d4_SendRawCmd_size                  270
#define dot15d4_SetNodeAddressCmd_size           13
#define dot15d4_SniffCmd_size                    6
#define dot15d4_StartCmd_size                    0
#define dot15d4_StopCmd_size                     0
#define dot15d4_SyncCmd_size                     22
#define dot15d4_WriteModifySuperframeCmd_size    29

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif
