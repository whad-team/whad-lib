...
/**
 * @brief   Parse a message as a PduReceived message.
 */

PduReceived::PduReceived(UnifyingMsg &message) : UnifyingMsg(message)
{
    this->unpack();
}
...
void PduReceived::addAddress(UnifyingAddress& address)
{
    m_hasAddress = true;
    m_address = address;
}
...
void PduReceived::unpack()
{
    ...
    result = whad_unifying_pdu_received_parse(
        this->getMessage(),
        &packet
    );
...
UnifyingPacket& PduReceived::getPacket()
{
    return m_packet;
}
...
UnifyingAddress& PduReceived::getAddress()
{
    return m_address;
}
