...
/**
 * @brief   Parse a message as a SendInSlotPdu message.
 */

SendInSlotPdu::SendInSlotPdu(Dot15d4Msg &message) : Dot15d4Msg(message)
{
    this->unpack();
}
...
/**
 * @brief       SendInSlotPdu message constructor.
 * 
...
SendInSlotPdu::SendInSlotPdu(uint64_t slot, uint32_t wait_offset, uint8_t *pPdu, int length) : Dot15d4Msg()
{
    /* Save slot. */
    m_slot = slot;
...
uint64_t SendInSlotPdu::getSlot()
{
    return m_slot;
}
...
/**
 * @brief   Retrieve the wait offsetbefore transmission
 * 
 * @retval  wait offset (in microseconds)
 */

uint32_t SendInSlotPdu::getWaitOffset()
{
