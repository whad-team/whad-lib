...
/**
 * @brief   Create a PduReceived message from a ZigbeeMsg message.
 * 
 * @param[in]   message     Reference to a ZigbeeMessage
 */

PduReceived::PduReceived(ZigbeeMsg &message) : ZigbeeMsg(message)
{
    this->unpack();
}
...
/**
 * @brief   Create a PduReceived message from a zigbee packet
 * 
 * @param[in]   packet      Zigbee packet to include in the message
 */

PduReceived::PduReceived(ZigbeePacket &packet)
{
    m_packet = packet;
}
...
ZigbeePacket& PduReceived::getPacket()
{
    return m_packet;
}